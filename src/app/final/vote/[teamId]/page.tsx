          ...
          {criteria.map((c) => (
            <div key={c.id}>
              ...
              <input
                ...
                min={0}
                max={c.max_score}
                // step 2 for the reduced sensitivity — but only when it
                // evenly divides max_score. Otherwise the 0-step grid
                // (0,2,4,...) never lands exactly on an odd max, so no one
                // could ever give (or reach) full marks on that criterion.
